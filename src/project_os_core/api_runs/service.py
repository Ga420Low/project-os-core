...
class ApiRunService:
    ...
    def execute_run(
        ...
        *,
        ...
        request = ApiRunRequest(
            ...
            skill_tags=context_pack.skill_tags,
            expected_outputs=list(expected_outputs or prompt_template.output_contract),
            metadata=dict(metadata or {}),
        )
    ...
    def monitor_snapshot(self, *, limit: int = 5) -> dict[str, Any]:
        rows = self.database.fetchall(
            """
            SELECT r.*, q.branch_name, q.objective
        ...
        for row in rows:
            created_at = str(row["created_at"])
            estimated_cost = float(row["estimated_cost_eur"])
            review_row = reviews.get(str(row["run_id"]))
            items.append(
                {
    ...
    def render_terminal_dashboard(self, *, limit: int = 5) -> str:
        ...
        for item in snapshot["latest_runs"]:
            lines.append(
                ...
                f"{item['estimated_cost_eur']:.4f} EUR | review={item['review_verdict'] or 'pending'}"
            )
    ...
    def _render_prompt_text(self, context_pack: ContextPack, template_config: dict[str, Any]) -> str:
        ...
        sections.append(
            ...
            "Return structured JSON only. Do not add prose outside the schema. Use the available context, call out missing facts explicitly, and prefer a repo/CLI-first implementation path."
        )
        return "\n\n".join(sections)
    ...
    def _default_constraints(self) -> list[str]:
        return [
            "Use repo/CLI first. Do not require Windows computer use for coding lane v1.",
            "Do not mutate main. Work against a codex/* branch.",
            "Do not bypass the Project OS mission router, runtime truth, or approval policy.",
            "Return structured JSON only and avoid hidden assumptions.",
        ]
